...
N=70000 #numbers of node in the sample
m= 10 #number of phases
K =20*int(np.log(N))#number of eigenvalues for the approximated heat kernel
...
file_name = "poisson_learning"

#if the weight matrix and eigenvectors/values were already computed before, load them
if(os.path.isfile("data/weights_mnist_"+file_name+".npz") and os.path.isfile("data/labels_mnist_"+file_name+".csv") and os.path.isfile("data/vals_mnist_"+file_name+".csv") and os.path.isfile("data/vecs_mnist_"+file_name+".csv")):
    W = sp.sparse.load_npz("data/weights_mnist_"+file_name+".npz")
    y = np.loadtxt("data/labels_mnist_"+file_name+".csv", delimiter=",", dtype=int)
    vals = np.loadtxt("data/vals_mnist_"+file_name+".csv", delimiter=",")
    vec = np.loadtxt("data/vecs_mnist_"+file_name+".csv", delimiter=",")
    ...
    #load weights from the graph learning library
    W = knn_costum('mnist', neighbours, metric='vae',kernel='gaussian', symmetrize=True)
    sp.sparse.save_npz("data/weights_mnist_"+file_name+".npz", W)

    #load labels
    y = gl.datasets.load('mnist', labels_only=True)
    np.savetxt("data/labels_mnist_"+file_name+".csv", y, fmt='%i', delimiter=",")
    
    #compute normalization of weight matrix
    D = np.sum(W, axis = 1).A1
    DW = sp.sparse.diags(1/D,0).dot(W)
    ...
    #compute eigenvalues/vectors of graph laplace
    vals, vec = eigs(DW, K)
    vec = vec.transpose().real
    vals = 1 - vals.real

    np.savetxt("data/vals_mnist_"+file_name+".csv", vals, delimiter=",")
    np.savetxt("data/vecs_mnist_"+file_name+".csv", vec, delimiter=",")
...
np.savetxt("results/mnist_"+str(temperature)+"_"+str(points_per_class)+"w^2-epsI.csv", s1s, delimiter=",")
np.savetxt("results/mnist_"+str(temperature)+"_"+str(points_per_class)+"I + h W + h^2 2 W^2.csv", s2s, delimiter=",")
np.savetxt("results/mnist_"+str(temperature)+"_"+str(points_per_class)+"W^2.csv", s3s, delimiter=",")
np.savetxt("results/mnist_"+str(temperature)+"_"+str(points_per_class)+"rank K.csv", s4s, delimiter=",")
np.savetxt("results/mnist_"+str(temperature)+"_"+str(points_per_class)+"W^4.csv", s5s, delimiter=",")

print("W^2 - eps I: ", s1/100)
print("I + h W + h^2/2 W^2: ", s2/100)
print("W^2: ", s3/100)
print("rank K: ", s4/100)
print("W^4: ", s5/100)
