...
def project_to_boundary(point, m):
    ...
    for i in range(3):
        c = point[int((i+2)%3)]/2
        boundary_point = point + c * (e(i, m)+e(int((i+1)%3), m)-2 *e(int((i+2)%3), m))
...
def draw_triangle_neu(median, neu, next_point, samples, clustering, m):
    ...
    for boundary_point in project_to_boundary(median, m):
        ax.plot([median[0],boundary_point[0]],[median[1],boundary_point[1]],[median[2],boundary_point[2]])
    for boundary_point in project_to_boundary(neu, m):
        ax.plot([neu[0],boundary_point[0]],[neu[1],boundary_point[1]],[neu[2],boundary_point[2]])
...
def fit_median(m, L, U, samples, median):
    ...
    for i in range(size):
        for j in range(m):
            ...
            if clustering[i]!=j:
                indices_along_normal[clustering[i]][j][-1] = len(sorted_along_normal[clustering[i]][j])
    ...
    for i in range(m):
        for j in range(m):
            if(i != j):
                t.heapify(sorted_along_normal[i][j], comperator(i,j,m), indices_along_normal[i][j], eti_along_normal[i][j])
    ...
    while(P!=count):
        ...
        while(diffs[index]>=0):
            ...
            popped,poppedindex = t.heappop(sorted_along_normal[index][predecessor], comperator(index, predecessor,m), indices_along_normal[index][predecessor], eti_along_normal[index][predecessor])
            ...
            for k in range(m):
                if(k!=index and k!=predecessor):
                    ...
                    #        location = i
                    t.delete(sorted_along_normal[index][k], indices_along_normal[index][k][poppedindex], comperator(index,k,m), indices_along_normal[index][k], eti_along_normal[index][k])
                if(k!=predecessor):
                    t.heappush(sorted_along_normal[predecessor][k], popped, comperator(predecessor, k,m), indices_along_normal[predecessor][k], poppedindex, eti_along_normal[predecessor][k])
            index = predecessor
    ...
    while(any(median[max_in_minus_indices[0]] > median[Jplus])):
        ...
        while(min_in_plus > median[max_in_minus_indices[0]]):
            ...
            if(len(max_non_T_minus_indices)>0):
                time_until_equality = (median[max_in_minus_indices[0]] - max_non_T_minus)/(1 + 1/(m-1))
            ...
            if(time_until_equality < time):
                ...
                ins.append(min_out)
                outs.remove(min_out)
                direction += 1/(m-1)*np.ones(m)-(1+1/(m-1))*e(min_out,m)
...
def test():
    ...
    samples = (np.random.uniform(0,1,m*size) + np.array([0.3,0,0.6,0]*size)).reshape((size,m))
