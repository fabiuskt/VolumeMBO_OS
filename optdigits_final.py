...
N=5620 #numbers of node in the sample
m= 10 #number of phases
K =2*int(np.log(N))  #number of eigenvalues for the approximated heat kernel
...
if(os.path.isfile("data/weights_optdigits.npz") and os.path.isfile("data/labels_optdigits.csv") and os.path.isfile("data/vals_optdigits.csv") and os.path.isfile("data/vecs_optdigits.csv")):
    W = sp.sparse.load_npz("data/weights_optdigits.npz")
    y = np.loadtxt("data/labels_optdigits.csv", delimiter=",", dtype=int)
    vals = np.loadtxt("data/vals_optdigits.csv", delimiter=",")
    vec = np.loadtxt("data/vecs_optdigits.csv", delimiter=",")
    ...
    #load OPT-Digits dataset
    x = []
    y = []
    file_name = 'data/optdigits.csv'
    with open(file_name, newline='') as csvfile:
        spamreader = csv.reader(csvfile, delimiter=',')
        for row in spamreader:
            y.append(int(row[-1]))
            x.append([int(numeric_string) for numeric_string in row[:-1]] )
    ...
    W = knn_costum(x, neighbours, kernel='gaussian', symmetrize=True)
    sp.sparse.save_npz("data/weights_optdigits.npz", W)
    np.savetxt("data/labels_optdigits.csv", y, fmt='%i', delimiter=",")
    ...
    #compute eigenvalues and vectors of graph Laplacian
    vals, vec = eigs(DW, K)
    vec = vec.transpose().real
    vals = 1 - vals.real

    #save values
    np.savetxt("data/vals_optdigits.csv", vals, delimiter=",")
    np.savetxt("data/vecs_optdigits.csv", vec, delimiter=",")
...
np.savetxt("results/optdigits_"+str(temperature)+"_"+str(points_per_class)+"w^2-epsI.csv", s1s, delimiter=",")
np.savetxt("results/optdigits_"+str(temperature)+"_"+str(points_per_class)+"I + h W + h^2 2 W^2.csv", s2s, delimiter=",")
np.savetxt("results/optdigits_"+str(temperature)+"_"+str(points_per_class)+"W^2.csv", s3s, delimiter=",")
np.savetxt("results/optdigits_"+str(temperature)+"_"+str(points_per_class)+"rank K.csv", s4s, delimiter=",")
np.savetxt("results/optdigits_"+str(temperature)+"_"+str(points_per_class)+"W^4.csv", s5s, delimiter=",")
